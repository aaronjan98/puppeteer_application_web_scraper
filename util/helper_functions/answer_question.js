'use strict'
const fs = require('fs')
const questions_file_path = '../indeed_questions.md'
...
function question_pool(question, answer_choices) {
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i]
            if (line.slice(0, 2) == '1.') { // if we're on a question
            ...
            } else if (line.length != 0 && line.trim().slice(0, 2) != '//' && found_question) { // make sure we're grabbing the answer and not a comment
                return line.trim()
